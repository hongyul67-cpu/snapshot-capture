...
DEFAULT_CONFIG = {
    ...
    "show_fixed_border": True,      # 고정 영역 빨간 테두리 항상 표시
    "video_fps": 15,
    "video_target": "전체화면",
    "hotkeys": {
        "region":   "<ctrl>+<shift>+1",
...
class CaptureApp:
    def __init__(self, root):
        ...
        self.fixed_border = None      # 고정 영역 상시 빨간 테두리
    ...
    def _build_ui(self):
        ...
        self.show_fixed_border_var = tk.BooleanVar(
            value=self.cfg.get("show_fixed_border", True))
        tk.Checkbutton(irow, text="🔴테두리", variable=self.show_fixed_border_var,
                       command=self._refresh_fixed_border, bg=C_CARD, fg=C_SUB,
                       selectcolor=C_CARD2, activebackground=C_CARD,
                       activeforeground=C_TEXT, font=("Segoe UI", 9),
                       highlightthickness=0, bd=0).pack(side="left", padx=(4, 0))
        ...
        Tooltip(self.mini_video_btn, "동영상 녹화 시작 / 정지")
        self.mini_interval_btn = self._mkbtn(bar, "▶", self.toggle_interval, hover=C_GREEN,
                                             font=("Segoe UI", 12, "bold"), pad=(0, 5))
        self.mini_interval_btn.pack(side="left", padx=2)
        Tooltip(self.mini_interval_btn, "연속 캡처 시작 / 정지")
        exp = self._mkbtn(bar, "⤢", self.toggle_mini, hover=C_ACCENT,
                          font=("Segoe UI", 12), pad=(0, 5))
        exp.pack(side="right", padx=2)
        Tooltip(exp, "원래 크기로 복원")
    ...
    def _refresh_fixed_border(self):
        """고정 영역이 있고 체크되어 있으면 상시 인터랙티브 빨간 테두리 표시/갱신"""
        if self.fixed_border:
            self.fixed_border.destroy()
            self.fixed_border = None
        try:
            on = self.show_fixed_border_var.get()
        except Exception:
            on = True
        if on and self.interval_region:
            try:
    ...
    def _on_fixed_region_changed(self, region):
        ...
        self.set_status(
            f"고정 영역 · {region['width']}×{region['height']} "
            f"@ ({region['left']},{region['top']})", C_SUB)

    def _hide_borders_for_grab(self):
        """캡처 순간, 화면의 빨간 테두리들이 이미지에 안 찍히게 잠깐 숨김"""
        hidden = []
        for b in (self.fixed_border, self.video_border, self.interval_border):
            if b:
                for s in b.strips:
                    try:
                        if s.winfo_ismapped():
    ...
    def _restore_borders(self, hidden):
        for s in hidden:
            try:
                s.deiconify()
    ...
    def _collect_cfg(self):
        ...
        self.cfg.update({
            ...
            "show_fixed_border": bool(self._safe_border_var()),
            "video_fps": fps,
    ...
    def _safe_border_var(self):
        try:
            return self.show_fixed_border_var.get()
        except Exception:
            return self.cfg.get("show_fixed_border", True)
    ...
    def _do_capture(self, region, flash=True):
        ...
        try:
            hidden = self._hide_borders_for_grab()
            try:
                img = grab_region(region)
            finally:
                self._restore_borders(hidden)
    ...
    def capture_region(self):
        ...
        self.root.withdraw()
        self.root.after(160, lambda: QuickSelector(self.root, self._on_region_selected))
    ...
    def _finish_region(self, region):
        ...
        self._refresh_fixed_border()  # 잠시 숨겼던 고정 테두리 복원
        self.root.deiconify()

    def set_fixed_region(self, then=None):
        """공유 '지정(고정) 영역'을 드래그로 지정. 이후 화면 테두리를 끌어 조정 가능."""
        self._suspend_fixed_border()
        self.root.withdraw()
        ...
        self.root.after(160, lambda: QuickSelector(self.root, cb))
    ...
    def start_video(self):
        ...
        if not (target == "지정 영역" and self.fixed_border):
            self.video_border = RedBorder(self.root, region)
    ...
    def start_interval(self):
        ...
        # 캡처 중인 영역에 빨간 테두리 표시 (고정 영역 상시 테두리가 있으면 중복 생략)
        if not (target == "지정 영역" and self.fixed_border):
    ...
    def on_close(self):
        ...
        if self.recording and self.recorder:
            self.recorder.stop()
        for b in (self.video_border, self.interval_border, self.fixed_border):
            if b:
                b.destroy()
        if self.hotkey_listener:
            try:
...
class Tooltip:
    ...
    def _show(self):
        ...
        lbl = tk.Label(self.tip, text=self.text, bg="#0b1220", fg=C_TEXT,
                       font=("Segoe UI", 9), padx=8, pady=4,
                       bd=1, relief="solid")
        lbl.pack()
        self.tip.update_idletasks()
        w = self.tip.winfo_width()
        self.tip.geometry(f"+{max(0, x - w // 2)}+{y}")


# ----------------------------------------------------------------------
# 빠른 영역 선택: 드래그 → 놓으면 즉시 확정, 드래그 중 픽셀 크기 표시
# ----------------------------------------------------------------------
class QuickSelector:
    def __init__(self, root, callback):
        ...
        self.canvas.create_text(v["width"] // 2, 40,
    ...
    def _up(self, e):
        ...
        if w < 5 or h < 5:
            self.callback(None)
            return
        self.callback({"left": self.vx + left, "top": self.vy + top,
                       "width": w, "height": h})
...
# ----------------------------------------------------------------------
# 화면 위 상시 '고정 영역' 테두리 — 테두리 드래그로 이동, 핸들로 크기변경
# (안쪽은 클릭 통과되어 아래 화면 그대로 사용 가능)
# ----------------------------------------------------------------------
class InteractiveRegion:
    ...
    def __init__(self, root, region, on_change=None):
        ...
        for name in ("top", "bottom", "left", "right"):
            w = self._mkwin(self.COLOR, "fleur")
        ...
        for name in ("nw", "ne", "sw", "se", "n", "s", "w", "e"):
            h = self._mkwin("#ffffff", self.CURSORS.get(name, "crosshair"))
            h.bind("<ButtonPress-1>", lambda e, n=name: self._start_resize(e, n))
    ...
    def _reposition(self):
        l = int(self.region["left"])
        t = int(self.region["top"])
        ww = int(self.region["width"])
        hh = int(self.region["height"])
        k, hs = self.STRIP, self.HS
        self.edges["top"].geometry(f"{ww + 2 * k}x{k}+{l - k}+{t - k}")
        self.edges["bottom"].geometry(f"{ww + 2 * k}x{k}+{l - k}+{t + hh}")
        self.edges["left"].geometry(f"{k}x{hh}+{l - k}+{t}")
        self.edges["right"].geometry(f"{k}x{hh}+{l + ww}+{t}")
        pts = {"nw": (l, t), "ne": (l + ww, t), "sw": (l, t + hh),
               "se": (l + ww, t + hh), "n": (l + ww // 2, t),
               "s": (l + ww // 2, t + hh), "w": (l, t + hh // 2),
               "e": (l + ww, t + hh // 2)}
    ...
    def _do_move(self, e):
        if not self._drag:
            return
        _, _, px, py, orig = self._drag
        dx, dy = e.x_root - px, e.y_root - py
        self.region["left"] = orig["left"] + dx
        self.region["top"] = orig["top"] + dy
        self._reposition()

    def _start_resize(self, e, name):
        self._drag = ("resize", name, e.x_root, e.y_root, dict(self.region))

    def _do_resize(self, e):
        if not self._drag:
            return
        _, name, px, py, orig = self._drag
        dx, dy = e.x_root - px, e.y_root - py
        x1, y1 = orig["left"], orig["top"]
        x2, y2 = x1 + orig["width"], y1 + orig["height"]
        if "n" in name:
            y1 += dy
        if "s" in name:
            y2 += dy
        ...
        if "e" in name:
            x2 += dx
        nl, nt = min(x1, x2), min(y1, y2)
        nw, nh = abs(x2 - x1), abs(y2 - y1)
        if nw >= 10 and nh >= 10:
            self.region = {"left": nl, "top": nt, "width": nw, "height": nh}
            self._reposition()

    def _end(self, e):
        self._drag = None
        if self.on_change:
            self.on_change(dict(self.region))
